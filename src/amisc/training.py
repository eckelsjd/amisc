...
@dataclass
class UncertaintySampling(TrainingData, PickleSerializable):
    """A class for storing training data in a sparse grid format. The `UncertaintySampling` class stores training points
    by their coordinate location in a larger tensor-product grid, and obtains new training data by refining
    a single 1d grid at a time. Refinement is done by Leja sampling for initial iterations, then using GP 
    uncertainty for subsequent iterations.
    ...
    !!! Info "About Uncertainty Sampling"
        Uncertainty Sampling is an active learning strategy that selects new training points based on the model's
        uncertainty. This approach aims to improve the surrogate model's accuracy by focusing on areas of the
        input space where the model is less certain about its predictions. The uncertainty is typically quantified
        using sklearn's GaussianProcessRegressor, which provides both mean predictions and uncertainty estimates.
    ...
    :ivar error_map: a `dict` of error information for each grid coordinate where the model failed
    :ivar latent_size: the number of latent coefficients for each variable (0 if scalar)
    :ivar d_thresh: minimum distance threshold for new points (specified as fraction of input domain size)
    :ivar thresh_relax: relaxation factor for minimum distance threshold in case no new points are found (threshold is 
    successively relaxed until number of points criteria is satisfied)
    :ivar grid_resolution: number of sample points per dimension for estimating GP uncertainty 
    (specified as multiple of number of new points to add)
    """
    MAX_IMPUTE_SIZE: ClassVar[int] = 10        # don't try to impute large arrays
    ...
    def refine(self, alpha: MultiIndex, beta: MultiIndex, input_domains: dict, gp_model, weight_fcns: dict = None):
        """Refine the sparse grid for a given `alpha` and `beta` pair and given collocation rules. Return any new
        grid points that do not have model evaluations saved yet.
        ...
        if gp_model is None: # Fallback to Leja sampling
            # Initialize a sparse grid for beta=(0, 0, ..., 0)
            if np.sum(beta) == 0:
                if len(self.x_grids) == 0:
                    num_latent = {}
            ...
            if np.sum(beta) == 0:
                if len(self.x_grids) == 0:
                    ...
                    for grid_size in self.beta_to_knots(beta):
                        if isinstance(grid_size, int):  # scalars
                            var, domain = next(domains)
                            new_pt[var] = self.sampling_1d(var, grid_size, input_domains, gp_model).tolist()
                        else:                           # latent coeffs
                            for s in grid_size:
                                var, domain = next(domains)
                                new_pt[var] = self.sampling_1d(var, s, input_domains, gp_model).tolist()
                    self.x_grids = new_pt
                self.betas.add(beta)
                self.yi_map.setdefault(alpha, dict())
                self.yi_nan_map.setdefault(alpha, dict())
                ...
                if self.is_one_level_refinement(beta_old, beta):
                    ...
                    for new_size in new_grid_size:
                        if isinstance(new_size, int):       # scalar grid
                            ...
                            if len(grid) < new_size:
                                num_new_pts = new_size - len(grid)
                                self.x_grids[var] = self.sampling_1d(var, num_new_pts, input_domains, 
                                                                     gp_model, grid).tolist()
                        else:                               # latent grid
                            for s_new in new_size:
                                var, grid, domain = next(inputs)
                                if len(grid) < s_new:
    ...
    def sampling_1d(self, var:str, N: int, input_domains:dict, gp_model, z_pts: np.ndarray = None) -> np.ndarray:
        """Find the next `N` points in the 1d sequence of `z_pts` using GP uncertainty sampling. 

        :param var: the variable to sample points for
        :param N: number of new points to add to the sequence
        :param input_domains: a `dict` of input variable domains
        :param gp_model: a trained GaussianProcessRegressor model for estimating uncertainty
        :param z_pts: current univariate sequence `(Nz,)`, start at middle of input_domains[var] if `None`
        :returns: the univariate sequence `z_pts` augmented by `N` new points
        """

        if z_pts is None:
            z_pts = np.array([0.5*(input_domains[var][0] + input_domains[var][1])])
            N = N - 1
            z_pts = np.atleast_1d(z_pts)
        else:
            if self.variance_criterion == 'local':
                ...
                def J1(x): 
                    # Local cost function
                    '''Optimize for highest variance at local subdomain'''
                ...
                constraints = []
                # Enforce minimumm distance constraint
                for xi in z_pts:
                ...
                def J2(x):
                    ...
                    '''Optimize for lowest posterior variance over a monte carlo test grid'''
                    var_order = list(self.x_grids.keys())
                    N = self.grid_size
                ...
                x0 = np.random.uniform(input_domains[var][0], input_domains[var][1], self.knots_per_level)
                res = minimize(J2, x0, bounds = [(input_domains[var][0], input_domains[var][1])] * self.knots_per_level)
                z_new = res.x
                z_pts = np.concatenate((z_pts, z_new))
